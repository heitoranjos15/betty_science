...
import (
	...
	"betty/science/app/bot"
	"betty/science/app/riot/clients/league"
	"betty/science/app/riot/core"
	"betty/science/app/riot/integrations"
	"betty/science/app/riot/repo"
	"betty/science/app/riot/worker"
...
type CmdSetup struct {
	...
	clientMatch     *league.ClientMatch
...
func (c *Command) defaultSetup() *CmdSetup {
	...
	clientMatch := league.NewClientMatch(api)
...
func (c *Command) RunMatch() {
	...
	botMatch := bot.NewBot(workerMatch, c.cfg)
	botMatch.Execute()
}

func (c *Command) RunGame() {
	setup := c.defaultSetup()
	clientGame := league.NewClientGame(setup.api)
	coreMatch := core.NewMatchCore(setup.matchRepository)
	coreGame := core.NewGameCore(clientGame, setup.gameRepository, setup.teamRepository, setup.matchRepository, repo.NewMongoRepo("players", c.mongo))
	workerGame := worker.NewWorkerGame(clientGame, coreGame, coreMatch)
	botGame := bot.NewBot(workerGame, c.cfg)
	botGame.Execute()
}

func (c *Command) RunFrame() {
	setup := c.defaultSetup()
	clientFrame := league.NewFramesClient(setup.api)
	...
	clientGame := league.NewClientGame(setup.api)
	coreGame := core.NewGameCore(clientGame, setup.gameRepository, setup.teamRepository, setup.matchRepository, playersRepository)
	workerFrame := worker.NewWorkerFrame(clientFrame, coreFrame, coreGame)
	botFrame := bot.NewBot(workerFrame, c.cfg)
	botFrame.Execute()
}

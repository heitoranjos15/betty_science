...
func loadMockGamesData(path string) league.FramesResponse {
	...
	var data league.FramesResponse
...
func loadMockPlayerFramesData(path string) league.PlayerFramesResponse {
	file, err := os.Open(path)
	if err != nil {
		log.Fatalf("Failed to open mock data file: %v", err)
	}
	...
	var data league.PlayerFramesResponse
	if err := json.NewDecoder(file).Decode(&data); err != nil {
		log.Fatalf("Failed to decode mock data: %v", err)
	}
	return data
...
func TestLoadFrames(t *testing.T) {
	...
	tests := []struct {
		...
		expectedFrame models.Frame
		expected      league.FrameResponse
		wantErr       bool
	}{
		{
			name: "Valid Data",
			...
			mock: func(mockClient *MockClient) {
				mockClient.On("GetFrames", "113503500599535523", gameEnd).Return(mockValid, league.ErrorGameFrameNoContent)
				mockClient.On("GetFrames", "113503500599535523", mock.Anything).Return(mockValid, nil)
				mockClient.On("GetPlayerFrames", "113503500599535523", mock.Anything).Return(mockPlayerValid, nil)
			},
			expectedFrame: frame,
			expected: league.FrameResponse{
				GameStart: gameStart,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			clientFrame := league.NewFramesClient(mockClient)
			...
			assert.Equal(t, tt.expected.GameStart, frames.GameStart)
			assert.Equal(t, tt.expected.GameEnd, frames.GameEnd)
			assert.Equal(t, len(tt.expected.Players), len(frames.Players))
			assert.Equal(t, tt.expected.Players, frames.Players)
			assert.Equal(t, tt.expected.WinnerID, frames.WinnerID)
		})
	}
}
